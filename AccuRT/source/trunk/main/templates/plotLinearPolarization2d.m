...
fileName1     = 'radiance_Q.txt';
fileName2     = 'radiance_U.txt';
runNo        = 1;
depthNo      = 1; 
wavelengthNo = 1;
...
data1 = readRadiance(fileName1);
data2 = readRadiance(fileName2);
...
for azimuthNo=1:data(runNo).nAzimuthAngles
    ...
    z1 = squeeze(data1(runNo).radiance(depthNo,wavelengthNo,:,azimuthNo))';
    z2 = squeeze(data2(runNo).radiance(depthNo,wavelengthNo,:,azimuthNo))';
    z = sqrt(z1.^2 + z2.^2)./z;
